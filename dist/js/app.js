        ...
        if (dropdowns.length) {
            ...
            if (window.matchMedia("(max-width: 1023px)").matches) {
                ...
                dropdowns.forEach(drop => {
                    ...
                    btn.addEventListener("click", e => {
                        e.stopPropagation();
                        if (!drop.classList.contains("_open")) drop.classList.add("_open"); else drop.classList.remove("_open");
                    });
                });
                function handleCloseAll() {
                    const openDrops = document.querySelectorAll(".dropdown._open");
                    if (openDrops) openDrops.forEach(d => d.classList.remove("_open"));
                }
            }
        }
    }
    ...
    function inputmask() {
        ...
        inputs.forEach(input => {
            ...
            input.addEventListener("keydown", e => {
                ...
                if (isFirstKey) {
                    if (e.key === "8") im_eight.mask(input); else if (e.key !== "Backspace") {
                        if (e.key === "7") e.preventDefault();
                        im_seven.mask(input);
                    }
                    isFirstKey = false;
                }
